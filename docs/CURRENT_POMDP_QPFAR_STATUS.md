...
| Q1 | Stale/partial CSI observation model (`current\|delay\|partial\|delay_partial`, age, mask) | `src/marl_topology/training/csi_observation_model.py` | **NOT_IMPLEMENTED** (only the `csi_age=0` placeholder exists) |
...
**Q0 — freeze D13 as the control baseline (in progress this round).** Then Q1 (stale/partial CSI
observation model), failing-test-first. See `docs/pomdp_qpfar/Q*/` for per-stage artifacts.
