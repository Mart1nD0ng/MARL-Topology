...
| **D8** | Re-test recurrent vs memoryless across {current-CSI, velocity, recurrent, velocity+recurrent} | **IN PROGRESS** — driver `dynamic_d8_matrix.py` (2×2 matrix on the corrected pipeline + D7 baseline groups) built + 2 tests (unit 680/0); pilot validated the matrix (flags active per arm); **5-seed headline RUNNING in background** (`dynamic_d8_matrix.json`). Verdict deferred to collection. Single-RSU scope. See `docs/dynamic_repair/D8/`. |
