...
def test_stage5_0k_documentation_exists_and_records_stage3_backing() -> None:
    text = _read_doc("STAGE5_0K_STAGE3_BACKED_FEASIBILITY_ENVELOPE_SWEEP.md")
    ...
    required = [
        ...
        "unknown_faults_remove_largest",
        "Stage 5.0l - Stage 3-backed sweep range expansion and realism review",
    ]
    missing = [item for item in required if item not in text]
    assert not missing, f"Stage 5.0k doc missing terms: {missing}"


def test_stage5_0k_harness_task_exists() -> None:
    path = ROOT / "harness" / "tasks" / "stage5_0k_stage3_backed_feasibility_envelope_sweep.yaml"
    task = yaml.safe_load(path.read_text(encoding="utf-8"))

    assert task["id"] == "stage5_0k_stage3_backed_feasibility_envelope_sweep"
    for field in [
        "required_artifacts",
        "expected_evidence",
        "negative_checks",
    ...
    assert "Stage 5.0j alpha probabilities" in negative_text
...
def test_stage5_0k_project_state_marks_completion_and_next_realism_review() -> None:
    ...
    missing = [item for item in required if item not in text]
    assert not missing, f"PROJECT_STATE missing Stage 5.0k state: {missing}"
