...
def test_stage3_micro_fixture_suite_review_doc_records_boundaries() -> None:
    text = (ROOT / "docs" / "STAGE3_MICRO_FIXTURE_SUITE_REVIEW.md").read_text(
        encoding="utf-8"
    )
    ...
    required = [
        ...
        "build_stage3_micro_fixture_suite_review",
        "stage3_micro_fixture_suite_report.py",
        "not a metric report",
        "not reward",
        "not training",
        "Full graph remains a baseline topology, not an oracle",
        "Stage 4 must define PBFT/application reliability separately",
    ]
    missing = [item for item in required if item not in text]
    assert not missing, f"Stage 3.5 review doc missing terms: {missing}"


def test_stage3_micro_fixture_suite_harness_task_exists() -> None:
    path = ROOT / "harness" / "tasks" / "stage3_micro_fixture_suite_review.yaml"
    task = yaml.safe_load(path.read_text(encoding="utf-8"))

    assert task["id"] == "stage3_micro_fixture_suite_review"
    for field in [
        "required_artifacts",
        "expected_evidence",
        "negative_checks",
...
def test_project_state_marks_stage3_5_complete_and_waits_for_owner() -> None:
    ...
    missing = [item for item in required if item not in text]
    assert not missing, f"PROJECT_STATE missing Stage 3.5 state: {missing}"
